...
function listing(type){const c=configs[type];return `<div class="welcome"><div><h1>${c.title}</h1><div class="muted">${c.kicker}</div></div><div class="welcome-actions"><button class="btn import-btn" onclick="openImportModal()">⇧ Import</button><button class="btn primary" onclick="openModal('${c.type}')">+ ${c.button}</button></div></div><div class="panel"><div class="toolbar"><input id="search" placeholder="${c.search}" oninput="filterTable()"><select id="statusFilter" onchange="filterTable()"><option value="">All statuses</option><option>Active</option><option>Pending</option><option>Issued</option><option>Published</option><option>Senior</option></select></div><div id="tableArea">${table(c.headers,c.rows(db[type]))}</div></div>`}
