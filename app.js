...
const active=db.residents.filter(x=>/senior|non-senior/i.test(x.status)).length;
...
<div class="metric-row"><span>Senior / Non-Senior</span><b>${active}</b><div class="bar"><i style="width:${active/total*100}%"></i></div></div>
</div>
...
function paymentDashboard(){
  ...
  db.payments.forEach(x=>{
    const d=x.date||'No date';
    groups[d]=(groups[d]||0)+(x.status==='Paid'?Number(x.amount||0):0);
  });
  ...
  const paymentBars=dates.map(d=>{
    const value=groups[d];
    const height=Math.max(value/max*100,value?8:2);
    return `<div class="payment-bar-item"><div class="payment-bar-value">PHP ${value.toLocaleString()}</div><div class="payment-bar-track"><i style="height:${height}%"></i></div><small>${esc(d)}</small></div>`;
  }).join('');
  const methods={};
  db.payments.forEach(x=>{methods[x.method]=(methods[x.method]||0)+1});
  const methodRows=Object.entries(methods).sort((a,b)=>b[1]-a[1]).map(([name,count])=>{
    return `<div class="insight-row"><span>${esc(name)}</span><b>${count}</b></div>`;
  }).join('');
  const reportCards=[
    ['Residents','Registered residents',db.residents.length],
    ['Households','Registered households',db.households.length],
    ['Documents','Document requests',db.documents.length],
    ['Payments','Payment records',db.payments.length],
    ['Collected','Paid amount','PHP '+totalPaid.toLocaleString()],
    ['Services','Active services',db.services.filter(x=>x.status==='Active').length]
  ].map(x=>`<div class="report-card"><div class="report-kicker">${x[0]}</div><p>${x[1]}</p><strong>${x[2]}</strong></div>`).join('');
  return `<div class="dashboard-insights">
    <div class="panel payment-chart">
      <div class="panel-head">
        <div><h3>Payment Records</h3><div class="muted">Paid collections by date</div></div>
        <button class="mini" onclick="go('payments')">View records</button>
      </div>
      <div class="payment-chart-total"><strong>PHP ${totalPaid.toLocaleString()}</strong><span>Total collected</span></div>
      <div class="payment-bars">${paymentBars||'<div class="empty">No payment records yet.</div>'}</div>
    </div>
    <div class="panel dashboard-analytics">
      <div class="panel-head"><div><h3>Payment Analytics</h3><div class="muted">Collection performance</div></div></div>
      <div class="insight-grid">
        <div><span>Paid records</span><strong>${paidCount}</strong></div>
        <div><span>Pending amount</span><strong>PHP ${pending.toLocaleString()}</strong></div>
        <div><span>Waived</span><strong>${db.payments.filter(x=>x.status==='Waived').length}</strong></div>
        <div><span>Methods used</span><strong>${Object.keys(methods).length}</strong></div>
      </div>
      <div class="insight-list"><h4>Payment Methods</h4>${methodRows||'<div class="empty">No payment methods recorded.</div>'}</div>
    </div>
  </div>
  <div class="section-heading">
    <div><h2>Reports Overview</h2><div class="muted">Quick administrative metrics from the current records.</div></div>
    <button class="mini" onclick="go('reports')">Open Reports</button>
  </div>
  <div class="report-grid dashboard-report-grid">${reportCards}</div>`;
}
...
return `${summary}<div class="welcome"><div><h1>${labels[kind]}</h1><div class="muted">Special sector registry and monitoring.</div></div><button class="btn primary" onclick="openModal('resident')">+ Add Resident</button></div>
...
function paymentSummary(){const paid=db.payments.filter(x=>x.status==='Paid').reduce((n,x)=>n+Number(x.amount||0),0);const pending=db.payments.filter(x=>x.status==='Pending').reduce((n,x)=>n+Number(x.amount||0),0);const waived=db.payments.filter(x=>x.status==='Waived').length;return `<div class="payment-summary"><div class="payment-stat"><span class="muted">Total Collected</span><strong>PHP ${paid.toLocaleString()}</strong></div><div class="payment-stat"><span class="muted">Pending Amount</span><strong>PHP ${pending.toLocaleString()}</strong></div><div class="payment-stat"><span class="muted">Payment Records</span><strong>${db.payments.length}</strong></div><div class="payment-stat"><span class="muted">Waived</span><strong>${waived}</strong></div></div>`}
function listing(type){const c=configs[type];const summary=type==='payments'?paymentSummary():'';return `<div class="welcome"><div><h1>${c.title}</h1><div class="muted">${c.kicker}</div></div><div class="welcome-actions">${type==='residents'?'<button class="btn import-btn" onclick="openImportModal()">⇧ Import</button>':''}<button class="btn primary" onclick="openModal('${c.type}')">+ ${c.button}</button></div></div><div class="panel"><div class="toolbar"><input id="search" placeholder="${c.search}" oninput="filterTable()"><select id="statusFilter" onchange="filterTable()"><option value="">All statuses</option><option>Non-Senior</option><option>Senior</option><option>Pending</option><option>Issued</option><option>Published</option><option>Senior</option></select></div><div id="tableArea">${table(c.headers,c.rows(db[type]))}</div></div>`}
